...
# Technical Documentation and Guidelines: PrimeNG & Supabase Stack

## 1. Database Architecture (Supabase)
The application utilizes a single Supabase project as the **Single Source of Truth** for both the relational database (PostgreSQL) and file archiving (Storage).

**Relational Database:** The schema is structured around four primary entities: `regions`, `tags`, `sub_tags`, and `content`. The `content` table serves as the main repository for the places submitted by the users.
* **Storage Management (Images):**
    * **Bucket:** `mrdtwice-images` (Visibility: Public).
    * **Security:** A Row Level Security (RLS) policy is actively enforced to allow `INSERT` operations for the `anon` (unauthenticated) role.
    * **Constraints:** Supported file formats are strictly limited to JPEG, PNG, and WEBP.
    * **Data Workflow:** The upload of binary files occurs **exclusively from the Frontend (Angular)** directly to Supabase Storage. Consequently, the Node.js backend APIs will only receive and persist the text-based public URL generated by Supabase within the `image` columns (belonging to the `regions` and `content` tables).

## 2. User Interface Guidelines (PrimeNG)
To ensure visual consistency throughout the MVP, the development of the user interface must strictly adhere to the utilization of the modules provided by the PrimeNG library.

**Imports:** Given the Angular architecture based on *Standalone Components*, each PrimeNG module must be individually imported within the `imports` array of the target component (e.g., `import { CardModule } from 'primeng/card';`).
**Designated Components:**
    * **Data Display:** Utilization of `p-card` for rendering the Place card grid.
    * **Form Input:** Utilization of `p-inputText`, `p-dropdown` (for the selection of `regions` and `tags`), and `p-inputTextarea` within the data entry forms.
    * **Reviews:** Utilization of the `p-rating` component for the visual input and consultation of user evaluations (stars).