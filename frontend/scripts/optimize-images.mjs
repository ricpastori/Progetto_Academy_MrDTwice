...
async function main() {
  const sources = await collectSourceImages(imageRoot);
  ...
  for (const sourcePath of sources) {
    const sourceRelative = toPosix(path.relative(imageRoot, sourcePath));
...
async function collectSourceImages(directory) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      // Never reprocess files generated by a previous run.
      if (absolutePath !== outputRoot) {
